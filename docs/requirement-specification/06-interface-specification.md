...
**Input:** `query` (string, required — arXiv `search_query` syntax: field prefixes `ti`/`au`/`abs`/`co`/`jr`/`cat`/`rn`/`all`, boolean `AND`/`OR`/`ANDNOT`), `max_results` (integer, optional, default 10), `start` (integer, optional, default 0), `sort_by` (`relevance` \| `lastUpdatedDate` \| `submittedDate`, default `relevance`), `sort_order` (`ascending` \| `descending`).
...
**Underlying call:** `GET .../{pmcid}/fullTextXML` (requires the PMCID form specifically, e.g. `PMC4767193` — confirmed to return a JATS `<article>` document with `<front>`/`<body>`/`<back>`). If `identifier` was given as `MED:{pmid}` rather than a PMCID, `fetch_metadata` is used first to resolve the corresponding `pmcid`; if the record has no `pmcid` or `full_text_available` is false, this fails with `format_unavailable` rather than attempting any of the third-party links in `fullTextUrlList`.
...
**Output on success:** `{"identifier": <canonical form>, "provider": "arxiv"|"europepmc", "resolved_url": <string>, "format": <string>}`.
