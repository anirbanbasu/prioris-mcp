...
A SQLite table, keyed by a unique constraint on `(provider, canonical_identifier, format, artefact)` with `INSERT ... ON CONFLICT` — see [ADR-00009: Catalogue as SQLite, not append-only JSONL-plus-replay](../ADR/00009-catalogue-sqlite-not-jsonl-replay.md) for why. See [Embedded SQLite vs. a client-server database](#embedded-sqlite-vs-a-client-server-database) below for the concurrency model this depends on, and its limits.
...
`catalogue.sqlite`, each document's `manifest.sqlite`, and `search.sqlite3` are separate SQLite files (the same engine, not the same file), even though they could technically share one — see [ADR-00012: Separate SQLite files, not one shared database](../ADR/00012-separate-sqlite-files-not-shared-database.md) for why, including the measured per-document overhead this costs.
