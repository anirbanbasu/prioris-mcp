...
`notes-search.sqlite3`'s FTS5 table holds just a note `id` (`UNINDEXED`) and tokenized `text`, not the denormalized `provider`/`identifier`/`format` columns `search.sqlite3` carries — see [ADR-00013: `notes-search.sqlite3` is deliberately minimal, not denormalized like `search.sqlite3`](../ADR/00013-notes-search-minimal-not-denormalized.md) for why. `search` applies every structured predicate directly against `notes.sqlite`; only if a keyword is given does it additionally intersect with the note `id`s matching `notes-search.sqlite3`.
...
`export` returns `{suggested_filename, frontmatter, markdown_body}` for exactly one note — **one file per note**, not one file per document — see [ADR-00014: Notes export is one file per note, not one file per document](../ADR/00014-notes-export-one-file-per-note.md) for why.
