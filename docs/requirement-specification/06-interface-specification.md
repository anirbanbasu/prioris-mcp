...
**Input:** `query` (string, required — FTS5 query syntax), `provider` (string, optional), `identifier` (string, optional — scopes to one document; requires `provider`), `format` (string, optional), `mode` (string, optional, default `"fts"` — any registered search mechanism name, e.g. `"fts"`, or `"hybrid"` for combined FTS + vector), `limit` (integer, optional, default `PRIORIS_MCP_VECTOR_SEARCH_DEFAULT_LIMIT` — applies to `vector` results, independent of `fts` result ranking).

**Behaviour:** runs `query` against the global FTS5 index via the [`SearchIndex`](01-architecture.md#searchindex) abstraction (see [Storage → Full-text search](storage/01-document-storage.md#full-text-search-the-searchsqlite3-index)) over persisted chunks — or, for a document with none, its leaves — applying whichever of `provider`/`identifier`/`format` are given as an additional `WHERE` filter alongside `MATCH`. When `mode` is `"hybrid"`, also runs a vector-based search in parallel, combining results. A query can legitimately match both a section and one of its own nested subsections, since every heading-nesting level is indexed as its own row — overlapping matches are expected, not deduplicated. Never triggers a fetch or parse; searching before anything has been persisted simply returns no results, not an error.

**Output:** `{"fts": [{"provider": <string>, "identifier": <string>, "format": <string>, "snippet": <string>, "offset": <int>, "score": <float>}, ...] | null, "vector": [{"provider": <string>, "identifier": <string>, "format": <string>, "chunk_id": <string>, "offset": <int>, "snippet": <string>, "score": <float>}, ...] | null, "index_status": {<mechanism_name>: <status_string>, ...}}` — `fts` is populated (a list, possibly empty) when `mode` is `"fts"` or `"hybrid"` — i.e. whenever the `fts` mechanism was actually invoked this call — and `null` otherwise (e.g. `mode="vector"`). `vector` follows the same rule for the `vector` mechanism: populated (a list, possibly empty, regardless of whether a vector index currently exists) when `mode` is `"vector"` or `"hybrid"`, and `null` otherwise. `null` means the mechanism wasn't invoked this call; an empty list means it was invoked but found no matches. Within `vector`'s entries, `score` is a cosine distance (0 = identical, larger = less similar — **lower is better**, the opposite of a typical relevance score), already sorted best-first in the returned list. `index_status` maps each search mechanism name (e.g. `"fts"`, `"vector"`) to a status string (`"ready"`, `"stale"`, or `"not_built"`), but is only populated when all three of `identifier`, `provider`, and `format` are given — otherwise it's an empty dict. `offset` in matched entries equals the matched entry's `span_start` in the document's own coordinate space, not an FTS5-internal offset — a caller can pass it straight to `parse_full_text`'s `offset` parameter to re-fetch surrounding context.
...
**Output:** `{"fts": {"notes": [<Note>, ...], "offset": <int>, "limit": <int>, "total": <int>, "has_more": <bool>} | null, "vector": [{"note_id": <string>, "text_preview": <string>, "score": <float>}, ...] | null, "index_status": {"vector": <status_string>} | null}` — `fts` is populated when `mode` is `"fts"` or `"hybrid"` (a `PagedNotes` object with pagination fields nested inside); `vector` is populated when `mode` is `"vector"` or `"hybrid"` and a `keyword` is provided. Within `vector`'s entries, `score` is a cosine distance (0 = identical, larger = less similar — **lower is better**, the opposite of a typical relevance score), already sorted best-first in the returned list. `index_status` is populated only when a vector search actually ran this call (mode in `("vector", "hybrid")` with keyword given), containing only the `"vector"` key with the worst-case status (`"ready"`, `"stale"`, or `"not_built"`) across every note actually returned in `vector` this call — there is no `fts` key in `index_status` since notes-FTS has no per-call existence check to report.
