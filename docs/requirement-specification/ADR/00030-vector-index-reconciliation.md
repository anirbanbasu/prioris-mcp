...
- **Blocking server startup on the full corpus reconciliation** — rejected: corpus size is unbounded, so this would make server startup latency a function of corpus size instead of a near-constant few milliseconds, for a background maintenance operation that doesn't need to complete before the server can usefully serve requests (existing content stays searchable at its current status throughout).
...
A same-dimension model rename is now also correctly detected as stale and reconciled, not just a dimension-changing one — closing the review's Low finding about same-dimension changes going undetected, alongside the review's High finding about the missing recovery path itself. Existing content stays at its current `index_status` throughout a reconciliation run rather than being blocked from being served — a caller polling `index_status` (or the new resource) sees `stale`/`building` progress toward `ready` rather than a frozen or unavailable corpus. Because reconciliation's progress counters are process-local, restarting mid-run loses visibility into "how far did that run get" — the next run recomputes its own total from scratch rather than resuming a persisted count; this is consistent with `building` status's existing non-persistence, not a new inconsistency.
