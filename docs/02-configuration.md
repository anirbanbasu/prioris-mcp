...
| `PRIORIS_MCP_LOCAL_FILE_ROOT` | Root directory `research_localfile_fetch_full_text` paths are resolved relative to — see [Security](requirement-specification/05-security.md#local-filesystem-access-is-confined-to-an-operator-configured-root). | The server process's current working directory | — |
| `PRIORIS_MCP_LOCAL_FILE_MAX_SIZE_BYTES` | Maximum size of a file `research_localfile_fetch_full_text` will read — see [Security](requirement-specification/05-security.md#fetched-content-is-untrusted-input-to-parse_full_text). | `10485760` (10MB) | integer, ≥ `1` |
