...
See [Graph search](search/03-graph-search.md) and its ADRs (00032–00041). No upstream API to ground against, like [local filesystem](#local-filesystem)/[storage management](#storage-management) below — this is a PriorisMCP design decision, verified here against `GraphSearchBackend`'s own typed method signatures rather than any third-party wire format.

**Conventions.** `NodeId`/`EdgeId` are both server-minted UUID strings. A `GraphNode` is a discriminated union on `kind`: a `PointerNode` (`{"kind": "pointer", "id", "ref_type": "chunk"|"document"|"note", "ref_id", "metadata", "created_at", "updated_at"}`) or a `ConceptNode` (`{"kind": "concept", "id", "label", "aliases", "description", "metadata", "created_at", "updated_at"}`) — `kind` is computed at read time from which node table matched, never a stored field (see [Node shape](search/03-graph-search.md#node-shape-two-typed-tables-pointer-and-concept)). A `GraphEdge` is `{"id", "from_id", "to_id", "relation_type", "weight": <float|null>, "metadata", "created_at", "updated_at"}`. A `GraphHop` pairs the two: `{"edge": <GraphEdge>, "node": <GraphNode>}`. `metadata` is always `dict[str, Any]` on read (never `null` — an empty bag reads back as `{}`) and `dict[str, Any] | None` on write; every write that may merge onto existing metadata (`upsert_pointer`, `update_concept`, `update_edge`) does so per-key, raising `metadata_conflict` on a differing value for an existing key (see [Conventions → Errors](#conventions) above and [Metadata merge-on-write](search/03-graph-search.md#metadata-merge-on-write-conflict-raises-not-silent-overwrite-or-merge)) rather than silently overwriting or merging it. `not_found` is raised for any op naming a `node_id`/`edge_id`/`from_id`/`to_id` that does not currently exist, except where noted otherwise below.

**Resource URIs.** `research://graph/concepts{?text,match,offset,limit}` and `research://graph/export{?format}` — see their own subsections below.

### `research_graph_write`

**Input:** `op` (required, one of `upsert_pointer`/`create_concept`/`update_concept`/`delete_node`/`create_edge`/`update_edge`/`delete_edge`) plus that op's own parameters — every parameter across every op is declared optional at the schema level (FastMCP derives one static schema per tool) and validated against the given `op`'s actual requirements at call time, the same runtime cross-field validation idiom `research_search_fetched` already uses for `author_filter`/`author_name`:

| `op` | Parameters | `id` in output |
|---|---|---|
| `upsert_pointer` | `ref_type` (`"chunk"`\|`"document"`\|`"note"`, required), `ref_id` (string, required), `metadata` (object, optional) | the `Pointer` node's id — an existing `(ref_type, ref_id)` pair upserts onto the same node, a new pair mints one |
| `create_concept` | `label` (string, required), `aliases` (list of strings, optional, default `[]`), `description` (string, optional), `metadata` (object, optional) | the newly minted `Concept` node's id — always a new node, never deduplicated server-side (see [`find_concepts`](#research_graph_query) below for caller-driven dedup) |
| `update_concept` | `node_id` (string, required), `label`/`aliases`/`description`/`metadata` (each optional; an unset field is left unchanged) | the updated node's id (echoes `node_id`) |
| `delete_node` | `node_id` (string, required) | the deleted node's id (echoes `node_id`) |
| `create_edge` | `from_id` (string, required), `to_id` (string, required), `relation_type` (string, required), `weight` (float, optional), `metadata` (object, optional) | the newly minted edge's id |
| `update_edge` | `edge_id` (string, required), `relation_type`/`weight`/`metadata` (each optional; an unset field is left unchanged) | the updated edge's id (echoes `edge_id`) |
| `delete_edge` | `edge_id` (string, required) | the deleted edge's id (echoes `edge_id`) |

**Behaviour:** dispatches directly onto the `GraphSearchBackend` method of the same name — no logic beyond op-based parameter validation and dispatch lives in the tool itself. `create_edge` resolves and validates both `from_id` and `to_id` exist before creating anything; a dangling edge referencing a nonexistent node is never created. `delete_node` cascades: every edge incident to the deleted node is removed too, with no separate report of which edges went with it — a subsequent `get_edge` on any of those edge ids fails `not_found`.

**Output:** `GraphWriteResult`: `{"op": <the requested op>, "id": <NodeId or EdgeId, per the table above>}`.

**Errors:** `invalid_request` for a call missing a required parameter for the given `op`, or an `op` outside the seven listed above; `not_found` for `update_concept`/`delete_node`/`create_edge` (either endpoint)/`update_edge`/`delete_edge` naming an id that does not exist; `metadata_conflict` for `upsert_pointer`/`update_concept`/`update_edge` per [Conventions](#conventions) above.

### `research_graph_query`

**Input:** `op` (required, one of `get_node`/`get_edge`/`neighbors`/`find_concepts`/`subgraph`) plus:

| `op` | Parameters |
|---|---|
| `get_node` | `node_id` (string, required) |
| `get_edge` | `edge_id` (string, required) |
| `neighbors` | `node_id` (string, required), `direction` (`"out"`\|`"in"`\|`"both"`, default `"both"`), `relation_type` (string, optional), `offset` (integer, optional, default 0), `limit` (integer, optional, default 50) |
| `find_concepts` | `query` (string, required), `limit` (integer, optional, default 20) |
| `subgraph` | `node_ids` (list of strings, required, one or more), `depth` (integer, optional, default 1), `relation_type` (string, optional) |

**Behaviour:** `get_node`/`get_edge` are direct lookups. `neighbors` pairs each connecting edge with its neighboring node (`GraphHop`, avoiding a second `get_edge` round trip per result) and does **not** raise `not_found` for a `node_id` absent from the graph — it returns an empty `matches` list, the same empty-not-error convention `research_search_fetched` uses for an unindexed query. `find_concepts` ranks every existing `Concept`'s `label`+`aliases` by `rapidfuzz` similarity against `query`, most-similar first — a recall-oriented candidate surfacer for the caller to judge, not a duplicate-blocking gate (see [Concept dedup](search/03-graph-search.md#concept-dedup-find_concepts-fuzzy-string-matching-not-exact-or-embedding-based)). `subgraph` returns every node/edge within `depth` hops of any given seed id, deduplicated across overlapping neighborhoods when multiple seeds are given, optionally restricted to one `relation_type`; like `neighbors`, a `node_ids` entry absent from the graph contributes nothing rather than raising `not_found`.

**Output:** `GraphQueryResult`, a `Field(discriminator="op")` union (wrapped under a top-level `"result"` key — see [Conventions → Output shapes](#conventions) above):

- `get_node` → `{"op": "get_node", "node": <GraphNode>}`
- `get_edge` → `{"op": "get_edge", "edge": <GraphEdge>}`
- `neighbors` → `{"op": "neighbors", "matches": [<GraphHop>, ...]}`
- `find_concepts` → `{"op": "find_concepts", "matches": [{"node": <ConceptNode>, "score": <float>}, ...]}`
- `subgraph` → `{"op": "subgraph", "nodes": [<GraphNode>, ...], "edges": [<GraphEdge>, ...]}`

**Errors:** `invalid_request` for a call missing a required parameter for the given `op`, or an `op` outside the five listed above; `not_found` for `get_node`/`get_edge` naming an id that does not exist (see [Behaviour](#research_graph_query) above for why `neighbors`/`subgraph` do not raise it).

### `research_graph_analyze`

**Input:** `op` (required, one of `betweenness_centrality`/`pagerank`/`communities`/`paths`/`reachable`/`steiner_tree`/`predict_links`) plus:

| `op` | Parameters |
|---|---|
| `betweenness_centrality` | `node_ids` (list of strings, required, one or more), `depth` (integer, optional, default 1), `relation_type` (string, optional) |
...
**Behaviour:** every op materializes a subgraph via `GraphSearchBackend.subgraph()`, seeded from the op's given node id(s) (`paths` seeds from both `from_id`/`to_id`) at the given `depth`/`max_depth`, then runs the named NetworkX algorithm over the resulting `MultiDiGraph` via `GraphAlgorithms` (see [Graph algorithm backend: NetworkX](search/03-graph-search.md#graph-algorithm-backend-networkx)). Every op is depth-bounded off an explicit seed set (or pair) — there is no op or parameter combination on this tool that runs over the whole graph unfiltered (that is `export_graph`'s deliberately separate purpose — see [`research://graph/export`](#researchgraphexport-resource) below). A seed id absent from the graph is not an error: it simply contributes no edges to the materialized subgraph, producing an empty or degenerate result (e.g. `scores: {}`, `communities: []`) for that op.

**Output:** `GraphAnalyzeResult`, a `Field(discriminator="op")` union (wrapped under a top-level `"result"` key):

- `betweenness_centrality`/`pagerank` → `{"op": <the requested op>, "scores": {<NodeId>: <float>, ...}}`
- `communities` → `{"op": "communities", "communities": [[<NodeId>, ...], ...]}`
- `paths` → `{"op": "paths", "paths": [[<GraphHop>, ...], ...]}` — each inner list is one simple path, hop-shaped the same as `neighbors`' `matches`
- `reachable` → `{"op": "reachable", "node_ids": [<NodeId>, ...]}`
- `steiner_tree` → `{"op": "steiner_tree", "nodes": [<GraphNode>, ...], "edges": [<GraphEdge>, ...]}`
- `predict_links` → `{"op": "predict_links", "predictions": [{"from_id": <NodeId>, "to_id": <NodeId>, "score": <float>}, ...]}`

**Errors:** `invalid_request` for a call missing a required parameter for the given `op` (e.g. `paths` without `max_depth`), or an `op` outside the seven listed above. No `not_found` case — see [Behaviour](#research_graph_analyze) above.

### Concept-vocabulary resource

**Resource URI:** `research://graph/concepts{?text,match,offset,limit}`.
...
**Resource URI:** `research://graph/export{?format}`.

**Input:** `format` (`"cypher_json"`\|`"graphml"`, default `"cypher_json"`) — a `GraphExportFormat`-typed query parameter; a value outside these two is rejected by FastMCP's own resource-template schema validation before the handler runs, not surfaced as a runtime `invalid_request`.

**Behaviour:** fetches the entire corpus-wide graph via `GraphSearchBackend.export_graph()` — every node and every edge, unfiltered, with no seed/depth bound (see [Diagnostic and visualization export](search/03-graph-search.md#diagnostic-and-visualization-export-export_graph)). `format="graphml"` additionally materializes the fetched dict into a NetworkX `MultiDiGraph` via `GraphAlgorithms.materialize()` and serialises it with `networkx.write_graphml`; since GraphML attribute values must be scalar, any `metadata` value that is itself a dict or list is JSON-stringified before serialisation (its already-JSON-encoded string leaf values, per [Metadata merge-on-write](search/03-graph-search.md#metadata-merge-on-write-conflict-raises-not-silent-overwrite-or-merge), pass through unchanged either way).
...
**Output:** `{"fts": {"notes": [<Note>, ...], "offset": <int>, "limit": <int>, "total": <int>, "has_more": <bool>} | null, "vector": {"matches": [{"note_id": <string>, "text_preview": <string>, "score": <float>}, ...], "offset": <int>, "limit": <int>, "total": <int>, "has_more": <bool>} | null, "index_status": {"vector": <IndexStatus>} | null}` — `fts` is populated when `mode` is `"fts"` or `"hybrid"` (a `PagedNotes` object with pagination fields nested inside); `vector` is populated when `mode` is `"vector"` or `"hybrid"` and a `keyword` is provided (a `PagedNoteVectorMatches` object, symmetric with `fts`'s `PagedNotes` — both nest their results list alongside `offset`/`limit`/`total`/`has_more`, rather than `vector` being a bare list the way it once was). Within `vector`'s entries, `score` is a cosine distance (0 = identical, larger = less similar — **lower is better**, the opposite of a typical relevance score), already sorted best-first in the returned list; `vector.has_more` is `offset + len(matches) < vector.total`, the same convention `fts` uses. `index_status` is populated only when a vector search actually ran this call (mode in `("vector", "hybrid")` with keyword given), containing only the `"vector"` key — typed as `IndexStatus`, the same shared `Literal["not_built", "stale", "ready", "building"]` `research_search_fetched` uses above, not a free-form string — with the worst-case status (`"building"` beats `"not_built"` beats `"stale"` beats `"ready"`) across every note id the caller's *structural filters* select (not merely the notes that ended up in `vector.matches` this page — an in-scope note with no vector row yet can never be a KNN match, but still needs to surface as `"building"`/`"not_built"` rather than being silently skipped), and across the whole corpus when no structural filter narrows the scope; an empty scope (no note matches the filters, or the corpus itself is empty) reports `"not_built"`. There is no `fts` key in `index_status` since notes-FTS has no per-call existence check to report.
