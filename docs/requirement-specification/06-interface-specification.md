...
**Output:** `{"fts": [{"provider": <string>, "identifier": <string>, "format": <string>, "snippet": <string>, "offset": <int>, "score": <float>}, ...] | null, "vector": [{"provider": <string>, "identifier": <string>, "format": <string>, "snippet": <string>, "offset": <int>, "score": <float>}, ...] | null, "index_status": {"fts_indexed": <bool>, "vector_indexed": <bool>, "embedding_model": <string|null>, "vector_index_stale": <bool>}}` — `fts` is populated when `mode` includes FTS (all modes currently), `vector` is populated when `mode` includes vector search and a vector index exists, both are `null` if the mode doesn't apply, and `index_status` reports indexing state across the corpus. `offset` in matched entries equals the matched entry's `span_start` in the document's own coordinate space, not an FTS5-internal offset — a caller can pass it straight to `parse_full_text`'s `offset` parameter to re-fetch surrounding context.
...
**Output:** `{"notes": [<Note>, ...], "offset": <int>, "limit": <int>, "total": <int>, "has_more": <bool>, "fts": {...} | null, "vector": [...] | null, "index_status": {...} | null}` — `notes` contains matching note objects, paginated; `fts`/`vector` carry search-result metadata from the search backend(s) used, both `null` when no `keyword` is provided; `index_status` is `null` when no vector index exists, otherwise `{"embedding_model": <string>, "indexed_count": <int>, "stale": <bool>}` reporting the corpus-wide state.
