...
This page maps the `ResearchPublicationProvider` capabilities defined in [Architecture](01-architecture.md) onto the concrete MCP tools and resources exposed for the three v1 sources — arXiv, Europe PMC, and the local filesystem — per the [storage](storage/01-document-storage.md) semantics already established. The local filesystem source implements a deliberately narrower subset of tools than arXiv/Europe PMC (see [Local filesystem tools](#local-filesystem-tools)); [Storage management tools](#storage-management-tools) are grouping-level, applying across all three.
...
| `research_delete_fetched` | Must accept **one or more** `(provider, identifier, format, artefact)` entries in a single call, where `artefact` is `document`, `markdown`, or `all` (see [Storage → Deletion is per-artefact, not per-format](storage/01-document-storage.md#deletion-is-per-artefact-not-per-format)); removes each matching persisted artefact, returning which were deleted and which were not found — it must not fail the whole call just because some requested entries no longer exist, the same partial-failure tolerance `fetch_metadata` already has. Deleting a `markdown` artefact also removes any extracted image artefacts anchored to it (see [Storage → Future: extracted PDF images](storage/01-document-storage.md#future-extracted-pdf-images)), when that capability is enabled. | Light |
| `research_search_fetched` | Must accept a free-text query plus optional `provider`/`identifier`/`format` filters; returns ranked matches (provider, identifier, format, and enough context — e.g. a snippet and an offset in the document's own coordinate space — to locate the match) against previously-persisted chunks, or a document's leaves as a fallback where it has no chunks (see [Storage → Full-text search](storage/01-document-storage.md#full-text-search-the-searchsqlite3-index)). A match names a specific section or page, not just "the document contains this term somewhere," and overlapping matches (a section and one of its own subsections both matching) are expected, not a defect. Must not trigger a fetch or parse. | Light |

`research_list_fetched`/`research_delete_fetched` are the primary, storage-abstraction-safe way to recover from a mistaken fetch (e.g. the wrong arXiv ID, or a local file fetched by accident) without depending on `StorageBackend` happening to be filesystem-based — see [Architecture → `list_fetched`/`delete_fetched`](01-architecture.md#list_fetched-delete_fetched-grouping-level) for why bypassing the abstraction (e.g. deleting files directly) isn't a substitute. Deletion is distinct from the deferred, disk-pressure-driven [retention/eviction](storage/01-document-storage.md#future-retention-and-redistribution-aware-persistence) policy, which v1 does not implement at all.
...
**v2** — see [SRS overview → Scope](index.md#v2) and [Architecture → `NotesBackend`](01-architecture.md#notesbackend). Unlike the per-provider tools above, these operate on `NotesBackend`, not `StorageBackend`, and are not provider-specific — a note can reference any provider's identifier, or none at all.

| Tool | Requirement | Cost |
|---|---|---|
| `research_notes_create` | Must accept `provider`, an identifier, an optional `format` (omitted for a note predating any fetch), the note `text`, and optional `anchors`/`author_name`/`tags`/`metadata`; resolves the identifier to its canonical/pinned form first (skipped when `format` is omitted — see [Notes storage → `NotesBackend`](storage/02-notes-storage.md#notesbackend)), then persists the note and returns it. Rejects an `anchors` entry with no location or selector content at all. | Light |
| `research_notes_read` | Must accept a note id; returns that note, or fails with "not found" if it doesn't exist. | Light |
| `research_notes_update` | Must accept a note id and any subset of `text`/`anchors`/`tags`/`metadata`; fields not given are left unchanged. `provider`/`canonical_identifier`/`format`/`author_name` cannot be changed by this tool — changing "which document/who" is a new note. Fails with "not found" if the id doesn't exist. | Light |
| `research_notes_delete` | Must accept a note id; removes it if present. Returns whether a note was actually removed rather than failing on an already-absent id. | Light |
| `research_notes_search` | Must accept any combination of `provider`/`canonical_identifier`/`format`/a date range/a keyword/an author filter/tag-membership filters, plus `offset`/`limit` pagination; no filters at all lists every note, paginated, newest first (see [Architecture → Listing is `search` with no filters](01-architecture.md#listing-is-search-with-no-filters-not-a-separate-tool)). A keyword orders results by relevance instead of recency. Fails with `invalid_request` if the author filter is set to "named" without also giving a name, or vice versa, or if `canonical_identifier` is given without `provider` (see [Notes storage → `search`](storage/02-notes-storage.md#search-is-structured-filtering-plus-optional-keyword-matching)). | Light |

`research_notes_create`/`research_notes_update` never trigger a fetch or parse — a note can exist for an identifier PriorisMCP has never fetched anything for.
...
For `provider=localfile`, `{identifier}` is the server-assigned caller-facing identifier `research_localfile_fetch_full_text` returned (see [Storage → Caller-facing identifiers](storage/01-document-storage.md#caller-facing-identifiers-for-sources-without-one)) — there is no server-side path to use instead, and even the optional `filename` hint wouldn't be segment-safe (it can contain `/`) or a stable identity for content the caller could later change.

`research://arxiv/categories` and `research://openalex/work-types` are different in kind: neither has a corresponding tool call or anything to persist to `StorageBackend` — each is a direct, response-cache-backed read of a provider-wide taxonomy (see [Architecture → Caching and rate limiting](01-architecture.md#caching-and-rate-limiting)), included as a resource rather than a tool because it's reference data to read, not an action with inputs to invoke.

Per-item metadata (title, authors, abstract, ...) is still **not** exposed as a resource: it's only ever response-cached (TTL-bound via `ResponseCachingMiddleware`), never written to `StorageBackend`, so there's no stable "it's just there" location for it the way there is for full text and Markdown — a metadata resource would be indistinguishable from just calling the tool again. `research://arxiv/categories` and `research://openalex/work-types` don't run into this problem because neither is per-item metadata; each is a single, provider-wide taxonomy lookup. The local filesystem source has no metadata resource at all, for the same reason it has no `fetch_metadata` tool (see [Local filesystem tools](#local-filesystem-tools)) — there's no authoritative metadata to expose in the first place.

## Extracted PDF images (optional)

Off by default, gated by `PRIORIS_MCP_PDF_EXTRACT_IMAGES` (see [Storage → Future: extracted PDF images](storage/01-document-storage.md#future-extracted-pdf-images)). This applies only to PDF full text parsed by `research_arxiv_parse_full_text` or `research_localfile_parse_full_text` — the two tools that go through the PDF parser backend — not JATS/HTML, which have no embedded image bytes available at parse time in either pipeline. `parse_full_text`'s own Markdown output is unaffected either way: an image is already represented there as an inline placeholder regardless of whether extraction is enabled. When enabled, an additional resource per extracted image becomes readable (URI scheme not yet decided — see [Security → Extracted PDF image resources must not leak filesystem paths](05-security.md#extracted-pdf-image-resources-must-not-leak-filesystem-paths)), and `research_delete_fetched` cascades to those image artefacts as described in [Storage management tools](#storage-management-tools) above. Search indexing does not cover images in either state — extracted images aren't text-searchable content.
