...
| `PRIORIS_MCP_PDF_OCR_TESSDATA_PATH` | Path to a pre-populated directory of Tesseract `.traineddata` files, for airgapped deployments — see [obtaining `.traineddata` files](#obtaining-traineddata-files-for-priorismcp_pdf_ocr_tessdata_path) below. Falls back to the standard `TESSDATA_PREFIX` if set and this is not. | unset (falls back to `TESSDATA_PREFIX`, then liteparse's own lazy-download behaviour) | — |
