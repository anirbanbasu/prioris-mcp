...
| `research_arxiv_parse_full_text` | An already-fetched `(arxiv_id, format)` returns `markdown` and `resource_uri`. A never-fetched `(arxiv_id, format)` fails with `not_found` and must not trigger a `fetch_full_text` call — assert no outbound network request occurs. For `format="pdf"`: a `page` request returns `total_pages` and a `page_range` consistent with `structure.jsonl`, and `offset` becomes relative to that page's start. `page` passed with `format="html"` fails with `invalid_request` before any storage read. |
...
| `research_search_fetched` | A query matching persisted `markdown` content returns ranked `matches`; a query matching nothing returns `matches: []`, not an error. An `identifier`+`provider`-scoped query only returns matches from that one document, even when other stored documents contain the same term. Never triggers a fetch or parse. Querying before anything has been persisted at all returns `matches: []`, not an error about a missing index. |

## Storage layer acceptance criteria

Per [Storage](02-storage.md), below the level of any single MCP tool:

- Two different formats fetched for the same `(provider, canonical identifier)` (e.g. arXiv `pdf` and `html`) land under the same `<document-hash>` directory, in separate `<format>/` subdirectories — asserted by fetching both and checking they share a document-hash prefix.
- `structure.jsonl` exists only for `format="pdf"`; it is absent for `format="html"`/Europe PMC XML entries.
- For a PDF with no tagged structure (including any OCR'd document — see [Security → OCR language data](05-security.md#ocr-language-data-is-a-network-dependency-of-parse_full_text)), `structure.jsonl` contains a flat page-only list, one entry per page, whose `{start, length}` ranges partition `markdown` with no gaps or overlaps and reconstruct each page's own text when sliced out.
- For a PDF with tagged structure (`liteparse`'s `extract_structure_tree`), `structure.jsonl` contains a nested tree with heading entries in addition to pages; a heading entry's `{start, length}` range, sliced out of `markdown`, must contain the heading's own text. A heading whose text cannot be located within its page's `markdown` slice is dropped from the tree, not included with a guessed/incorrect range — assert the tree is smaller, not wrong.
- `catalogue.jsonl` reflects every `write`/`delete` exactly once, in order; replaying it from empty reproduces the same set of live entries `list`/`exists`/`find_canonical_identifier` report.
- The FTS5 index (`search.sqlite3`) can be deleted and is transparently rebuilt from `catalogue.jsonl` + `markdown` files on next use, with no error surfaced to the caller and no change in `research_search_fetched` results before/after.
- A pre-existing flat `<hash>`/`<hash>.json` store (the layout that predates this design) migrates into `documents/<document-hash>/<format>/...` plus a freshly-built `catalogue.jsonl`, and the migration is a no-op (idempotent) when run a second time against an already-migrated store, and safe against a store that's a mix of old and new layout.
