...
**Output:** `{"fts": {"notes": [<Note>, ...], "offset": <int>, "limit": <int>, "total": <int>, "has_more": <bool>} | null, "vector": [{"note_id": <string>, "text_preview": <string>, "score": <float>}, ...] | null, "index_status": {"vector": <status_string>} | null}` — `fts` is populated when `mode` is `"fts"` or `"hybrid"` (a `PagedNotes` object with pagination fields nested inside); `vector` is populated when `mode` is `"vector"` or `"hybrid"` and a `keyword` is provided. `index_status` is populated only when a vector search actually ran this call (mode in `("vector", "hybrid")` with keyword given), containing only the `"vector"` key with the worst-case status (`"ready"`, `"stale"`, or `"not_built"`) across every note actually returned in `vector` this call — there is no `fts` key in `index_status` since notes-FTS has no per-call existence check to report.
