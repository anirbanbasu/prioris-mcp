...
| Tool | Description | Key inputs | Notes |
|---|---|---|---|
| `research_list_fetched` | Enumerate persisted `(provider, identifier, format, artefact)` catalogue entries, read from `catalogue.sqlite`. | `provider` (optional), `format` (optional) | Never triggers a fetch or parse. Omitting both filters lists every persisted entry across all three sources. `artefact` is `document` or `markdown` — the fetched source and its parsed Markdown are listed as separate entries, each with its own `fetched_at_or_parsed_at`/`size_bytes`. |
...
| `research_search_fetched` | Full-text search over previously-persisted chunks (or leaves, for documents with none). | `query` (FTS5 syntax), `provider` (optional), `identifier` (optional, requires `provider`), `format` (optional) | Runs against the global SQLite FTS5 index (`search.sqlite3`), ranked by `bm25()` (most relevant first). Never triggers a fetch or parse — searching before anything is persisted just returns no results. `identifier` without `provider` fails with `invalid_request`, as does malformed FTS5 query syntax. Each match's `offset` is in the document's own coordinate space and can be passed straight to the matching `parse_full_text` tool's `offset` to fetch surrounding context. |

Grouping-level, like `research_resolve_identifier` — not split per provider, since none of the three tools validates anything provider-specific (see [Architecture → `list_fetched`/`delete_fetched`](requirement-specification/01-architecture.md#list_fetched-delete_fetched-grouping-level)).
...
| `research_notes_search` | Search/list notes; structured filters plus optional FTS5 keyword search over note text. | `provider`, `canonical_identifier`, `format`, `date_from`/`date_to` (ISO 8601), `keyword` (FTS5 syntax), `author_filter` (`any`\|`mine`\|`named`), `author_name`, `tags_all`/`tags_any`/`tags_exclude`, `offset`/`limit` (paged, newest first) | No filters at all returns everything, paged. `canonical_identifier` requires `provider`; `author_filter="named"` requires `author_name` and vice versa; both fail with `invalid_request`. `date_from`/`date_to` are validated as ISO 8601 and normalised to UTC before comparison — an unparseable value fails with `invalid_request`. Malformed FTS5 syntax in `keyword` also fails with `invalid_request`, same as `research_search_fetched`. |
