...
- `catalogue.sqlite` reflects every `write`/`delete` exactly once; a unique-constraint violation on `(provider, canonical_identifier, format)` — simulating two concurrent local writer processes racing to write the same key — resolves via `INSERT ... ON CONFLICT`, not a crash or a duplicate row, and `list`/`exists`/`find_canonical_identifier` report exactly one live entry for that key afterward.
